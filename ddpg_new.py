...
class DDPG(object):
    def __init__(self, a_dim, s_dim, s_length, a_bound, tau=0.01, memory_cap=1024, batch_size=128, gamma=0.9,
                 lr_a=0.001,
                 lr_c=0.003):
    ...
    @property
    def can_train(self):
        return self.pointer >= self.memory_cap
    ...
    def _build_c(self, s, a, reuse=None, custom_getter=None):
        ...
        with tf.variable_scope('Critic', reuse=reuse, custom_getter=custom_getter):
            ...
            w1_s = tf.get_variable('w1_s', [self.s_dim_, n_l1], trainable=trainable)
    ...
    def __build_state(self, s, trainable=True):
        # conv0 = tf.layers.conv1d(s, 64, 9, activation='tanh', trainable=trainable)
        # dp0 = tf.layers.dropout(conv0, 0.5, name="state_dp2")
        # conv1 = tf.layers.conv1d(dp0, 32, 9, activation='tanh', trainable=trainable)
        # dp1 = tf.layers.dropout(conv1, 0.5, name="state_dp2")
        # conv2 = tf.layers.conv1d(dp1, 32, 9, activation='relu', trainable=trainable)
        ...
        with tf.variable_scope('STATE', reuse=tf.AUTO_REUSE):
            ...
            dense1 = tf.layers.dense(lstm, units=128, activation=tf.nn.relu, name='lstm_dense1_s')
            dense2 = tf.layers.dense(dense1, units=256, name='lstm_dense2_s')
        return dense2
