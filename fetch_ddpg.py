...
INSTRUMENT = "USDJPY"
EFFCIENCY_PERIOD = 60 * 24 * 3
INITIAL_CASH = 1000000
START = datetime.datetime(2014, 1, 1, 0, 0, 0)
END = datetime.datetime(2015, 1, 1, 0, 0,0)

brain = DDPG.DDPG(2, s_dim=9, s_length=EFFCIENCY_PERIOD, a_bound=1.0, memory_cap=2048, batch_size=256)
"""
...
class MyStrategy(strategy.BaseStrategy):
    ...
    def onExitOk(self, position: strategy.position.Position):
        ...
        if done % 100 == 0:
            rate = (self.profit_trade / done) if done is not 0 else 0
            self.logger.info(
                'cash:%.3f\teq:%.3f\t trade done:%d win_rate:%.3f w:%d l:%d' % (
...
bar_feed = BarFeed(bar=fetch_data(''))
