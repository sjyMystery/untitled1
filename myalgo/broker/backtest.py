...
class BackTestBroker(Subject):
    ...
    def equity(self):
        ...
        for instrument, shares in six.iteritems(self.__quantities):
            if shares > 0:
                instrument_bar = self.__bar_feed.last_bar(instrument)
                assert instrument_bar is not None, "Price for %s is missing" % instrument
                ret += instrument_bar.out_price * shares
        return ret
